...
def create_server(
    title: str = "MCP Server to manage DockerHub and Github accounts",
    description: str = "Server with command autodiscovery support to manage DockerHub and Github accounts",
    ...
    # Setup logging
    setup_logging()
    logger = get_logger("mcp_empty_server")
    
    # Discover and register commands from our package
    logger.info("Starting command autodiscovery...")
    command_registry.discover_commands("mcp_empty_server.commands")
...
def run_server(
    ...
    **server_kwargs
    ...
    print("=" * 80)
    print("🚀 MCP EMPTY SERVER")
    print("=" * 80)
    print(f"📋 Description: Empty server with command autodiscovery")
    ...
    logger.info(f"Starting MCP Empty Server on {host}:{port}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="MCP Empty Server")
